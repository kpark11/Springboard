...
# Create initial LogisticRegression model
gb = GBTClassifier(maxIter=5, maxDepth=2, labelCol="label", seed=42, leafCol="leafId")
...
gbParamGrid = (ParamGridBuilder()
               .addGrid(gb.maxIter,[1,3,5,7,10])
               .addGrid(gb.maxDepth,[1,2,3,4,5])
               .build())
...
attr = list(lrCvPredictions.schema['features'].metadata['ml_attr']['attrs'].values())
attrs = attr[0] + attr[1]
pd_attrs = pd.DataFrame([(attr['name'],lrCvModel.bestModel.coefficients[attr['idx']]) for attr in attrs],columns=['name','coefficient'])
pd_attrs.sort_values(by='coefficient',ascending=False)

# COMMAND ----------

gbCvFeatureImportance = pd.DataFrame([(attr['name'], gbCvModel.bestModel.featureImportances[attr['idx']]) for attr in attrs],columns=['feature_name','feature_importance'])
